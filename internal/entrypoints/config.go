...
func resolveRepoConfig(_ schema.TediumConfig, targetRepo schema.Repo) (schema.ResolvedRepoConfig, error) {
	...
	for {
		...
		platform := platforms.FromURL(configRepo.PlatformBaseURL)
		if platform == nil {
			return schema.ResolvedRepoConfig{}, fmt.Errorf("failed to determine a platform to read repo config (config URL: %s, platform domain: %s)", configUrl, configRepo.PlatformBaseURL)
		}
	...
	for souceChoreIdx, sourceChore := range mergedConfig.Chores {
		...
		platform := platforms.FromURL(choreRepo.PlatformBaseURL)
		if platform == nil {
			return schema.ResolvedRepoConfig{}, fmt.Errorf("failed to determine a platform to read chore config (domain: %s)", choreRepo.PlatformBaseURL)
		}
